...
import Data.Attoparsec.Text (IResult(..), Parser, Result, endOfInput, parse, takeTill)
...
parseDate = do
  ...
  let time  = secondsToDiffTime $ (unsafeDecimal hour) * (60 * 60) + (unsafeDecimal minute) * (60) + (unsafeDecimal sec)
...
getCommits = do
  ...
  case maybeResults of
    Left  err     -> return $ Fail empty [] err
    Right results -> return . parse (many $ parseLogLine <* endOfInput) . pack $ results
